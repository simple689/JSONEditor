...
WidgetHistory.init = function () {
    ...
    if (!WidgetHistory._localStorage) {
        LogBase.log('This browser does not support localStorage');
    }
...
WidgetHistory.addFile = function (fileName, fileContent, contentType, historyItemFile) {
    ...
    if (!fileEditItem) {
        fileEditItem = '{"' + APIData._fileList + '":[{"' + KeyBase._fileName + '":"首页","' + KeyBase._fileContent + '":"首页内容"}]}';
    }
    ...
    for (var o in fileList) {
        var key = o;
        if (fileList[key][KeyBase._fileName] == fileName) {
            isExist = true;
            var fileStr = fileContent;
            if (contentType == WidgetTab._enumAddContentType.fileJsonObj) {
                fileStr = JSON.stringify(fileContent);
            }
            fileList[key][KeyBase._fileContent] = fileStr;
            break;
        }
    }
    if (!isExist) {
        var newFileJson = '{"' + KeyBase._fileName + '":"' + fileName + '","' + KeyBase._fileContent + '":""}';
        fileList.push(JSON.parse(newFileJson));
        var len = fileList.length;
        fileList[len - 1][KeyBase._fileContent] = fileContent;
    }
...
WidgetHistory.existFileBrowserFile = function (jsonObj, key, extend) {
    ...
    for (var i in list) {
        var obj = list[i];
        if (obj[KeyBase._key] == key && obj[APIData._extend] == extend) {
            return true;
        }
    }
    return false;
