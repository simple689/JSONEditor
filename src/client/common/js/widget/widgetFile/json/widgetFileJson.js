...
WidgetFileJson.callbackInitMould = function (initMouldType) {
    ...
    // 通过jsonMd创建json
    widgetFileJson.readMould(widgetFileJson._widgetFileJsonMould._jsonMouldObj[KeyBase._file], widgetFileJson._jsonObj, "", widgetFileJson._rootDd, false);
}
...
WidgetFileJson.prototype.readMould = function (jsonObjMd, jsonObj, keyParent, elementParent, isListParent) {
    for (var keyMd in jsonObjMd) {
        ...
        if (typeof(valueMd) == KeyBase._object) {
...
WidgetFileJson.prototype.readMouldKeyTypeObject = function (jsonObjMd, jsonObj, keyParent, elementParent, isListParent, key) {
    var valueMd = jsonObjMd[KeyBase._value];
    ...
    jsonObjCtrl._keyShow = jsonObjMd[KeyBase._showTitle];
    if (isListParent) {
...
WidgetFileJson.prototype.readMouldKeyTypeArray = function (jsonObjMd, jsonObj, keyParent, elementParent, isListParent, key) {
    ...
    for (var key in value) {
        ...
        jsonObjCtrl._keyShow = valueMd[KeyBase._showTitle] + "_";
        var keyShowIndex = parseInt(key) + 1;
        jsonObjCtrl._keyShow += keyShowIndex;
        jsonObjCtrl._type = KeyBase._arrayItem;
        jsonObjCtrl._onMouseEnter = WidgetFileMenu.onMenu;
...
WidgetFileJson.prototype.readMouldKeyTypeEnum = function (jsonObjMd, jsonObj, keyParent, elementParent, isListParent, key) {
    ...
    var enumType = value[KeyBase._enumType];
    if ((enumType && enumType.length <= 0) || !enumType) {
        value[KeyBase._enumType] = valueMd[KeyBase._enumTypeDefault];
        enumType = value[KeyBase._enumType];
    }
    jsonObjCtrl._value = enumType;

    var jsonListCtrl = new JsonListCtrl(0);
    var enumList = valueMd[KeyBase._enumList];
    for (var oItemMd in enumList) {
        var valueItemMd = enumList[oItemMd];
        var item = new JsonListItem(oItemMd, valueItemMd[KeyBase._showTitle]);
        item._paramList = valueItemMd[KeyBase._enumParamList];
    ...
    var jsonEnumParamList = enumList[enumType][KeyBase._enumParamList];
    if (jsonEnumParamList) {
        for (var oItemMd in jsonEnumParamList) {
            var valueItemMd = jsonEnumParamList[oItemMd];
            this.readMouldKey(valueItemMd, value[KeyBase._enumParamList], keyParent, dd, false, oItemMd);
        }
    }
}
...
WidgetFileJson.listAdd = function (jsonObjCtrl) {
    ...
    for (var oItemMd in jsonObjCtrl._objMd[KeyBase._value]) {
        var valueItemMd = jsonObjCtrl._objMd[KeyBase._value][oItemMd];
        list.push("");
...
WidgetFileJson.a = function (jsonObjMd, jsonObj, key) {
    ...
    if (valueTypeMd == KeyBase._object) {
        ...
        jsonObj[key] = {}
        var enumTypeDefaultMd = valueMd[KeyBase._enumTypeDefault];
        var enumListMd = valueMd[KeyBase._enumList];

        var enumItemMd = enumListMd[enumTypeDefaultMd];
        if (enumListMd) {
            var jsonObjEnum = jsonObj[key];
            jsonObjEnum[KeyBase._enumType] = enumTypeDefaultMd;
            jsonObjEnum[KeyBase._enumParamList] = {};
            for (var oItemMd in enumItemMd[KeyBase._enumParamList]) {
                var valueItemMd = enumItemMd[KeyBase._enumParamList][oItemMd];
                WidgetFileJson.a(valueItemMd, jsonObjEnum[KeyBase._enumParamList], oItemMd);
            }
        }
    } else if (valueTypeMd == KeyBase._link) {
        jsonObj[key] = "";
