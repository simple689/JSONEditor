...
WidgetMenuFold.setDdDisplay = function (dd, labelCheck, isCheck) {
    //展开和收齐的不同状态下更换小图标
    if (isCheck) {
        dd.style.display = KeyBase._block;
        $(labelCheck).css(
            "background-image", "url(" + confCommonRoot + "/img/widget/widgetMenuFold/menuFold_arrowBottom.jpg)"
        );
    } else {
        dd.style.display = KeyBase._none;
        $(labelCheck).css(
            "background-image", "url(" + confCommonRoot + "/img/widget/widgetMenuFold/menuFold_arrowTop.jpg)"
        );
    }
}
